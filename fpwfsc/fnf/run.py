...
def run(camera=None, aosystem=None, config=None, configspec=None,
    ...
    if hitchhiker_mode==True:
        Hitch = hh.Hitchhiker(imagedir=hitchhiker_path)
    ...
    #adding in aperature rotation     
    rotation_angle_threshold = 5 # If determined the aperature has rotated more than 5 degrees, re-generate reference aperature
    ...
    for i in np.arange(Niter):
        ...
        rotation_angle_deg_cur = rotation_angle_aperture #12/02 need to get telescope telemetry? or rotation measurement in the loop
        if np.abs(rotation_angle_deg_pre-rotation_angle_deg_cur) > rotation_angle_threshold:
            ...
            # plt.savefig(f'/usr/local/home/cguthery/PyAO/guis/fast_and_furious/GUI2/FPWFSC/fpwfsc/fnf/focalimg{i}.png')

        if hitchhiker_mode==True:
            #XXX for hitchhiker, we need to output the file name, shoild be in _read_fits, but other places are using the hitchiker so...
            img= Hitch.wait_for_next_image(stop_event = my_event, timeout= None)
            if img is None:
                print('Hitch Hiker Stopped')
                break
        else:
            #filename = Camera.get_next_filename()
