...
if __name__ == "__main__":
    camera = 'Sim'
    aosystem = 'Sim'
    hwconfig='../speckle_suppression/sn_config.ini' 
    hwconfigspec='../speckle_suppression/sn_config.spec'
    config='satellite_config.ini'
    configspec='satellite_config.spec'

    config_file = config
    hwsettings = sf.validate_config(hwconfig, hwconfigspec)
    ...
    searchrad = 60
    radtol = 20
    setpoint1 = np.array([425,330])
    PIDloop = PID(Kp=0.3, Ki=0.1, Kd = 0.0, setpoint=setpoint1)
    ...
    for i in range(100):
        drift = dm.generate_tip_tilt(initial_shape.shape, tilt_x = -200e-9, tilt_y=0, 
                                          dm_rotation=35, flipx=False)
        current_shape = AOSystem.get_dm_data()
        data_speck_raw = Camera.take_image()
        data_speck = sf.equalize_image(data_speck_raw, **bgds)
